...
Context(TextProgram){
    Spec(Size_0_HotDays){
        const int size = 0;
        float weather[size];
        AssertThrows(std::logic_error, countConsecutiveHotDays(weather, size, 100));
    }
    Spec(Size_1_HotDays){
        const int size = 1;
        float weather[size] = {100.5};
        Assert::That(countConsecutiveHotDays(weather, size, 100), Equals(1));
    }
    Spec(Size_7_HotDays){
        const int size = 7;
        float weather[size] = {90.3, 96.5, 106.6, 108.3, 104.4, 98.9, 103.0};
        Assert::That(countConsecutiveHotDays(weather, size, 100.5), Equals(3));
    }
    Spec(Size_14_HotDays){
        const int size = 14;
        float weather[size] = {102.9, 95.9, 91.1, 95.9, 95.9, 101.4, 107.4, 106.9, 101.6, 99.8, 97.0, 90.2, 109.3, 95.9};
        Assert::That(countConsecutiveHotDays(weather, size, 99.8), Equals(5));
    }
    Spec(Size_31_HotDays){
        ...
        Assert::That(countConsecutiveHotDays(weather, size, 100.3), Equals(10));
    }
};
