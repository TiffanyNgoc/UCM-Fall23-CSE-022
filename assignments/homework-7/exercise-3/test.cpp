...
Context(TextProgram){
    Spec(Size_0_MaxChange){
        const int size = 0;
        float weather[size];
        AssertThrows(std::logic_error, getMaxChange(weather, size));
    }
    Spec(Size_1_MaxChange){
        const int size = 1;
        float weather[size] = {100.5};
        AssertThrows(std::logic_error, getMaxChange(weather, size));
    }
    Spec(Size_7_MaxChange){
        const int size = 7;
        float weather[size] = {90.3, 96.5, 106.6, 108.3, 104.4, 98.9, 103.0};
        Assert::That(getMaxChange(weather, size), EqualsWithDelta(10.1, 0.001));
    }
    Spec(Size_14_MaxChange){
        const int size = 14;
        float weather[size] = {102.9, 95.9, 91.1, 95.9, 95.9, 101.4, 107.4, 106.9, 101.6, 99.8, 97.0, 90.2, 109.3, 95.9};
        Assert::That(getMaxChange(weather, size), EqualsWithDelta(19.1, 0.001));
    }
    Spec(Size_31_MaxChange){
        const int size = 31;
        ...
        Assert::That(getMaxChange(weather, size), EqualsWithDelta(15.8, 0.001));
    }
};
